...
abc_dict = {'a': 0, 'b': 1, 'c': 2, 'd': 3, 'e': 4, 'f': 5, 'g': 6, 'h': 7, 'i': 8, 'j': 9,
...
αβγ_dict = {'α': 0, 'β': 1, 'γ': 2, 'δ': 3, 'ε': 4, 'ζ': 5, 'η': 6, 'θ': 7, 'ι': 8, 'κ': 9, 'λ': 10, 'μ': 11, 'ν': 12,
            'ξ': 13, 'ο': 14, 'π': 15, 'ρ': 16, 'σ': 17, 'τ': 18, 'υ': 19, 'φ': 20, 'χ': 21, 'ψ': 22, 'ω': 23, 'ά': 0,
            'έ': 4, 'ή': 6, 'ί': 8, 'ϊ': 8, 'ΐ': 8, 'ό': 14, 'ς': 17, 'ύ': 19, 'ϋ': 19, 'ΰ': 19, 'ώ': 23}
ΑΛΦΑ = 'ΑΒΓΔΕΖΗΘΙΚΛΜΝΞΟΠΡΣΤΥΦΧΨΩ'
    ...
    for decrypted_message in Dict:
        ...
        for i in range(1, 4):
            ...
            for j in range(len(decrypted_message)-i):
                if decrypted_message[j: j+i+1] in Ngrams:
                    points += Ngrams[decrypted_message[j: j+i+1]]
